...
def get_pergunta(user):
    Account_Checklist = str(request.args['id-checklist'])
    ...
    try:
        ...
        response = jsonify({'erro': 'Erro. Não foi possível acessar esse Item dessa conta.'})
        return response, 500
...
def get_all_perguntas(user):
    ...
    if user['uid'] != User_id:
        response = jsonify({'erro': 'Erro. Não foi possível acessar os Itens dessa conta.'})
        return response, 500
...
def edit_pergunta(user):
    try:
        ...
        try:
            ...
            pass
        response = jsonify({'erro':'Editado com sucesso.'})
        return response, 200
    except Exception as e:
        response = jsonify({'erro':'Não foi possível editar o item.' + str(e)})
        return response, 500
...
def create_pergunta(user):
    try:
        ...
        try:
            ...
            pass
        response = jsonify({'erro':'Item criado com sucesso.'})
        return response, 200
    except Exception as e:
        response = jsonify({'erro':'Não foi possível criar um novo Item. Mais detalhes: ' + str(e)})
        return response, 500